...
def inertia_tensor(Ixx, Iyy, Izz, Ixy, Ixz, Iyz):
    I = cs.MX.zeros(3, 3)
...
def quat_to_R_T(R_T, quat):
    return G(quat) @ E_T(quat)


# Return a python list with the phase of each spline in the trajectory
def get_force_spline_phases(**args):
    col_point_phases = []
...
# Return a list of the surations of each force spline in the trajectory
def get_force_spline_durations(**args):
    durations = []
...
# Return a python list with the phase of each ee pos spline in the trajectory
def get_ee_spline_phases(**args):
    col_point_phases = []
    ...
    for i in range(num_phases):
        if is_contact == True:
            col_point_phases += [True]
        else:
            num_splines = args["num_splines_per_contact"]
            col_point_phases += [False for _ in range(num_splines)]
        is_contact = not is_contact

    return col_point_phases


# Return a list of the surations of each ee spline in the trajectory
def get_ee_spline_durations(**args):
    durations = []
    ...
    for i in range(num_phases):
        if is_contact == True:
            durations += [args["contact_durations"][contact_idx]]
            contact_idx += 1
        else:
            num_splines = args["num_splines_per_contact"]
            durations += [
                args["swing_durations"][swing_idx] / num_splines
                for _ in range(num_splines)
            ]
            swing_idx += 1
        is_contact = not is_contact
...
def get_contact_forces(
    ...
    if is_curr_contact:
        ...
        is_next_contact = spline_is_contact[spl_idx + 1] if is_not_last else False
